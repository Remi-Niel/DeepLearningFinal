...
class Solver(object):
    ...
    def build_model(self):
        ...
        img_rec = self.G([ez_output, ey_output_])


        # img_rec = self.G([ez_output_fake, ey_output_])
        # [ez_output_rec, ey_output_rec] = self.E(img_rec)

        y_input = tf.keras.layers.Input([5])
        self.Ez = tf.keras.Model(y_input,y_input)
        ey_output_fake = self.Ez(ey_output_fake)

        z_input = tf.keras.layers.Input([400])
        self.Ez = tf.keras.Model(z_input,z_input)
        ez_output_fake = self.Ez(ez_output_fake)
        ...
        self.gan = tf.keras.Model(inputs = [img, orig_labels, target_labels], outputs = [output_cls, ey_output, ez_output_fake, img_rec, ey_output_fake])
        self.gan.compile(loss=['binary_crossentropy','mse','mse','mae','mse'], optimizer=self.g_optimizer, loss_weights = [1, 1, 1, 1, 1])
        self.gan.summary()
    ...
    def train(self):
        ...
        with tf.keras.backend.get_session().as_default(): 
            ...
            image = image / 255.
            image = image * 2 - 1
            x_input = image.reshape(1,self.image_size,self.image_size,3)

            label_input = np.asarray([1,0,0,1,0])
            label_input = label_input.reshape(1,1,1,5)
            ...
            outcome = cv2.cvtColor(outcome, cv2.COLOR_RGB2BGR)
            # plt.imshow(outcome)
            # plt.show()
        ...
        #             d_labels = np.concatenate([d_real_labels, d_fake_labels])
        #             d_input = np.concatenate([x_real,x_fake])
        #             d_input_labels = np.concatenate([label_org,label_trg])

        #             self.shuffle_in_unison(d_input, d_input_labels, d_labels)

        #             d_loss = self.D.train_on_batch([d_input, d_input_labels],[d_labels])

        #         [z_, y_] = self.E.predict(x_real)

        #         g_loss = self.gan.train_on_batch([x_real,label_org_,label_trg_],[d_real_labels,label_org,z_,x_real,label_trg])
